...
import {PROJECTS, getMenuRef} from "../../constants";
...
const ProjectsSection = ({isDesktop}: IDesktop) => {
  ...
  }, [targetSectionRef, sectionTitleElementRef, isDesktop, hasProjects]);
  ...
  const renderSectionTitle = (): React.ReactNode => (
    ...
    >
      ...
      <h1 className="section-heading seq mt-2">Building in Public</h1>
      <h2 className="text-2xl md:max-w-3xl w-full seq max-w-sm mt-2">
        MERN, AI/ML, and open-source explorations that will be documented here
        soon.
      </h2>
    </div>
  );
  ...
  return (
    <section
      ...
      className={`${isDesktop && hasProjects ? "min-h-screen" : ""} ${
        PROJECT_STYLES.SECTION
      }`}
      id={getMenuRef("Projects")}
    >
      {renderSectionTitle()}
      {hasProjects ? (
        ...
        <div className="mt-8 rounded-3xl border border-gray-700/70 bg-gray-800/40 p-8 text-lg">
